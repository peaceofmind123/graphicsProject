...
Model* Components::loadModel(const string& path )
{
    ...
    while(std::getline(fileBuffer,line))
    {
        ...
        size_t foundNormal = line.find("vn");
        size_t foundSurface = line.find("f");
        bool isFoundVertex = (foundVertex==0);
        bool isFoundNormal = (foundNormal==0) && (foundVertex!=0);
        bool isFoundSurface = (foundSurface==0);
        bool isFound = isFoundVertex || isFoundSurface || isFoundNormal;
        if(isFound)
        {
            do
            {
                firstCharPos = line.find_first_not_of(" ");
                lastCharPos = line.find_last_not_of(" ");

                line = line.substr(firstCharPos);
                if(!(lastCharPos<line.length()-2))
                    line=line.substr(0,lastCharPos+1);

            } while(firstCharPos!=0 || lastCharPos!=line.length()-1);
...
Surface* Components::parseSurface(const vector<string>& tokens,Model* model)
{
    ...
    {
        bool skipMappingFlag=false; //triggered when vertex normal info is not available
        ...
        else if(numSlashes<2) //no vertexnormal info available, so mapping need not be done
        {
            skipMappingFlag=true;
            try
            {
                 k = std::stoi(subTokens[0]);
            }
        ...
        else //case entered when number of slashes exceeds 2
        {
            throw BadFileException();
        }
        try
